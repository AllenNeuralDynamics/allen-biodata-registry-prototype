"""Pytest fixtures for the Migration Runner Lambda unit tests.

We add the parent directory to ``sys.path`` so the tests can import
``runner`` and ``handler`` directly without a package-style install —
the same approach AWS Lambda uses to load entry-point modules from the
deployment zip's root.
"""
