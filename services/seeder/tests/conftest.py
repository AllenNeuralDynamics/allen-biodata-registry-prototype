"""Pytest fixtures for the Seeder Lambda unit tests.

We add the parent directory to ``sys.path`` so the tests can import
``handler``, ``seeder``, and ``mapping`` directly without a package-
style install — the same approach AWS Lambda uses to load entry-point
...
# handler.py, seeder.py, and mapping.py.
