###############################################################################
# Allen BioData Registry PoC — lambdas/authorizer module
#
# Provisions:
#   * A Python 3.12 Lambda function packaged from
#     services/authorizer-lambda/. The packaging step pip-installs the
#     runtime deps (PyJWT, cryptography, psycopg[binary]) into a build
#     directory, copies handler.py alongside, and zips the result.
...
#   * VPC config so the Lambda can reach Aurora's private subnets AND
#     egress to Cognito's public JWKS endpoint.
#
# Validates: R9.7, R14.4, R19.4, R19.5 — the Authorizer Lambda
# implementation that validates Cognito JWTs and resolves the auth
# context from Aurora on every API request.
#
# Design references:
...
#
# Wiring:
#   The apigateway module (Task 14.1) consumes `function_arn` and
#   `function_name` outputs to wire the REQUEST authorizer + invoke
#   permission. authorizer_result_ttl_in_seconds = 300 is configured
#   on that module's aws_api_gateway_authorizer.cognito resource.
###############################################################################
...
locals {
  ...
  build_dir   = coalesce(var.build_dir, "${path.root}/.terraform/biodata-registry/${var.name_prefix}-authorizer-build")
  package_dir = "${local.build_dir}/package"
  zip_path    = "${local.build_dir}/authorizer.zip"
...
#
# Hash the requirements file and every .py file under the source
# directory (excluding tests/). Any change in either bumps the hash and
# triggers null_resource.package to rebuild on next apply.
###############################################################################
